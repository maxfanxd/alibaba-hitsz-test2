...
def parse_prediction(text: str, candidates: Optional[List[str]], max_items: int) -> List[str]:
    ...
    if end_idx != -1:
        text = text[:end_idx]
    # Normalize separators: Chinese comma、顿号、English comma/semicolon/newline
    text = text.strip()
...
def main():
    parser = argparse.ArgumentParser(description='使用LoRA适配器对验证集进行推理并生成提交格式JSON')
    ...
    parser.add_argument('--lora_path', default='checkpoints/qwen2.5-7b-instruct-lora', type=str)
    ...
    out_path = os.path.join(args.output_dir, f'{name}_lora.json')
